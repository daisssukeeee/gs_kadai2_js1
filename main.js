...
window.onload = function () {
  ...
  var menuItems = document.querySelectorAll('.sp-nav a'); // 全てのリンクを取得
  var meatImage = document.getElementById('meat-image'); // 肉の画像要素

  hamburger.addEventListener('click', function () {
      nav.classList.toggle('open');
      // ハンバーガーメニューがクリックされたら肉の画像を表示
      meatImage.style.display = nav.classList.contains('open') ? 'block' : 'none';
  });
  ...
  blackBg.addEventListener('click', function () {
    ...
    // メニューのリンクがクリックされたときにメニューを閉じ、肉の画像を隠す
    menuItems.forEach(function (menuItem) {
      menuItem.addEventListener('click', function () {
        nav.classList.remove('open');
        meatImage.style.display = 'none';
      });
    });
  };
